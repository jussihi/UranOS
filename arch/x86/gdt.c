...
void gdt_init()
{
  ...
  insert_segment_descriptor(&gdt[1], 0, 0xfffff, 0x9A, GDT_GRAN_PAGE);

  /* Kernel data descriptor */
  insert_segment_descriptor(&gdt[2], 0, 0xfffff, 0x92, GDT_GRAN_PAGE);

  /* Userland ring3 code descriptor */
  insert_segment_descriptor(&gdt[3], 0, 0xfffff, 0xFA, GDT_GRAN_PAGE);

  /* Userland ring3 data descriptor */
  insert_segment_descriptor(&gdt[4], 0, 0xfffff, 0xF2, GDT_GRAN_PAGE);

  /* TODO: TSS descriptor */
  // insert_segment_descriptor(&gdt[5], /* something */, 0xfffff, int type, GDT_GRAN_BYTE);

  /* Refresh the GDT pointer */
  uint64_t gdtr_descriptor = make_gdtr_descriptor(sizeof(gdt) - 1, gdt);
  asm volatile ("lgdt %0" : : "m" (gdtr_descriptor));
